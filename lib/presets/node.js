...
import { SuperJSONFile, SuperJSONFileSync, } from '../adapters/node/SuperJSONFile.js';
...
export async function SuperJSONFilePreset(filename, defaultData) {
    const adapter = process.env.NODE_ENV === 'test'
        ? new Memory()
        : new SuperJSONFile(filename);
    const db = new Low(adapter, defaultData);
    await db.read();
    return db;
}
export function SuperJSONFileSyncPreset(filename, defaultData) {
    const adapter = process.env.NODE_ENV === 'test'
        ? new MemorySync()
        : new SuperJSONFileSync(filename);
    const db = new LowSync(adapter, defaultData);
    db.read();
    return db;
}
